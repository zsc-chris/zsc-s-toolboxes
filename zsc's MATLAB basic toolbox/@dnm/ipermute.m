...
%IPERMUTE	Inversely permute dimensions' names of dnm.
%	ret=IPERMUTE(self,dims,finalize) returns a permuted dnm where the
%	dimensions are inversely permuted from the original dims. The value
%	will be returned if finalize is true, which allows one to directly feed
...
%	Copyright 2025 Chris H. Zhao
	...
	zsc.assert(all(~ismissing(dims))&&isequal(sort(index(self.dimnames,dims)),1:numel(dims)),message("MATLAB:permute:badIndex"));
	out=outclass;
	ret=dnm(zsc.ipermute(self.value,index(self.dimnames,dims)),dims(out{2,2,@sort,index(self.dimnames,dims)}));
	if finalize
		ret=gather(ret);
	end
end