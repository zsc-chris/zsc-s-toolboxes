function ret=movmad(self,k,dims,nanflag,options)
	...
	dstar=dstarclass;
	ret=applykernel(self,@(x,dims,nanflag)median(abs(x-median(x,dims,nanflag)),dims,nanflag),flatten(zsc.cat(1,num2cell(dims),k)),dstar{options},mode="dist",additionalinput={nanflag});
end